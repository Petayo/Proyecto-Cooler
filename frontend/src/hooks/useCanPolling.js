...
import { normalizeCanPayload } from '../utils/normalizeCan';
  ...
  useEffect(() => {
    ...
    const fetchPredictions = async () => {
      try {
        ...
        const normalized = normalizeCanPayload(payload);
        ...
        setEvents((previous) => {
          ...
          const next = [...previous, ...fresh].slice(-APP_CONFIG.maxCanEvents);

          if (seenFingerprints.current.size > APP_CONFIG.maxCanEvents * 3) {
            const retained = new Set(next.map((event) => buildEventFingerprint(event)));
